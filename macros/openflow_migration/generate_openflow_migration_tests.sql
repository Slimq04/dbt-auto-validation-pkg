{#-
  Prints schema.yml snippets for Openflow migration validation.
-#}
...
{% macro generate_openflow_migration_tests(
  ...
  {% if stage in ['raw_ingestion', 'raw_snapshotting'] %}
    ...
    {% do lines.append('    description: Openflow migration validation (' ~ stage ~ ')') %}
    {% do lines.append('    tests:') %}
    {% do lines.append('      - openflow_migration_no_duplicates:') %}
    ...
    {% if baseline is not none %}
      ...
      {% if (_gb | length) > 0 and ((_sum | length) > 0 or (_cnt | length) > 0) %}
        {% do lines.append('      - openflow_migration_compare_queries:') %}
  ...
  {% elif stage == 'modeling_downstream' %}
    {% do lines.append('  - name: ' ~ model_name) %}
    {% do lines.append('    description: Openflow migration validation (modeling_downstream)') %}
    {% do lines.append('    tests:') %}
    {% do lines.append('      - openflow_migration_no_duplicates:') %}
    {% do lines.append('          composite_key_columns:') %}
    {% for c in composite_key_columns %}
    ...
    {% endfor %}
    {% if baseline is not none %}
      {% do lines.append('      - openflow_migration_compare_columns:') %}
      {% do lines.append('          baseline: ' ~ baseline) %}
    {% endif %}
    {% if time_column is not none %}
      {% do lines.append('      - openflow_migration_no_data_gaps_calendar:') %}
